...
def letterMatrix(sizeX, sizeY):
    ...
    while sizeX > 0:
        column = array('c', empty * sizeY)   #a row with 0 in each column
...
def testNewWord(x, y, hor, word, matrix):
    ...
    if hor:
        ...
        while i < j:
            ...
            if a == empty:
                #checking above
                if y != 0 and matrix[x+i][y-1] != empty:
                    return False
                #checking beneath
                if y+1 != len(matrix[0]) and matrix[x+i][y+1] != empty:
                    return False
    ...
    #now checking vertical
        ...
        while i < j:
            ...
            if a == empty:
                ...
                if x != 0 and matrix[x-1][y+i] != empty:
                    return False
                #checking beneath
                if x+1 != len(matrix) and matrix[x+1][y+i] != empty:
                    return False
